            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">作物状态展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">作物阶段</label>
                          <div class="layui-input-inline">
                              <select name="zuowuzhuangtaiJieduanTypes" id="zuowuzhuangtaiJieduanTypes" lay-filter="zuowuzhuangtaiJieduanTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in zuowuzhuangtaiJieduanTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">作物状态</label>
                          <div class="layui-input-inline">
                              <select name="zuowuzhuangtaiTypes" id="zuowuzhuangtaiTypes" lay-filter="zuowuzhuangtaiTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in zuowuzhuangtaiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            zuowuzhuangtaiJieduanTypesList : [],
            zuowuzhuangtaiTypesList : [],
            zuowuzhuangtaiTypes: 1,//类型
            limit: 10,
        ...
        filters: {
            //把 zuowuzhuangtaiContent 字段去除无用字符 如果字段长度大于60  就截取到60
            zuowuzhuangtaiContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            var zuowuzhuangtaiJieduanTypes = $("#zuowuzhuangtaiJieduanTypes").find("option:selected").val();
            if (zuowuzhuangtaiJieduanTypes != null && zuowuzhuangtaiJieduanTypes != "" && zuowuzhuangtaiJieduanTypes != "null") {
                param['zuowuzhuangtaiJieduanTypes'] = zuowuzhuangtaiJieduanTypes;
            }
         
            var zuowuzhuangtaiTypes = $("#zuowuzhuangtaiTypes").find("option:selected").val();
            if (zuowuzhuangtaiTypes != null && zuowuzhuangtaiTypes != "" && zuowuzhuangtaiTypes != "null") {
                param['zuowuzhuangtaiTypes'] = zuowuzhuangtaiTypes;
            }
                                            // 获取列表数据
            http.request('zuowuzhuangtai/list', 'get', param, function(res) {
    ...
    // 当前表的字典表
       ...
       function zuowuzhuangtaiJieduanTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=zuowuzhuangtai_jieduan_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.zuowuzhuangtaiJieduanTypesList = res.data.list;
               }
           });
       }
       ...
       function zuowuzhuangtaiTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=zuowuzhuangtai_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.zuowuzhuangtaiTypesList = res.data.list;
               }
           });
       }
