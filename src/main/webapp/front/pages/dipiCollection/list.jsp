            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">地皮收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="dipiName" id="dipiName" placeholder="地皮名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="dipiWeizhi" id="dipiWeizhi" placeholder="地皮位置" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../dipi/detail.jsp?id='+item.dipiId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.dipiPhoto?item.dipiPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../dipi/detail.jsp?id='+item.dipiId)"><span>{{item.dipiName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.dipiNewMoney"><span style="color: red;">{{item.dipiNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteDipiCollection(item.id)">删除 {{item.dipiName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 dipiCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            dipiCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteDipiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('dipiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#dipiName').val() != null && jquery('#dipiName').val() != "" && jquery('#dipiName').val() != "null" ) {
                param['dipiName'] = jquery('#dipiName').val();
            }
                     
            if (jquery('#dipiWeizhi').val() != null && jquery('#dipiWeizhi').val() != "" && jquery('#dipiWeizhi').val() != "null" ) {
                param['dipiWeizhi'] = jquery('#dipiWeizhi').val();
            }
                                                                                                    
            // 当前表
                                                    // 获取列表数据
            http.request('dipiCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function dipiTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=dipi_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.dipiTypesList = res.data.list;
               }
           });
       }
