...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的地皮订单 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <%--<ul class="layui-tab-title">--%>
                    <%--<li v-for="item in zuowuTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>--%>
                    <%--<li v-for="item in dipiOrderYesnoTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>--%>
                ...
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.dipiName}}</td>
                            <td style="width: 60px;">{{item.dipiWeizhi}}</td>
                            <td style="width: 250px;">
                                <img :src="item.dipiPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 100px;color: red">{{item.dipiNewMoney}}</td>
                            <td style="width: 80px;">{{item.dipiValue}}</td>
                            <td style="width: 150px;">{{item.zuowuName}}</td>
                            <td style="width: 80px;">{{item.zuowuValue}}</td>
                            <td style="width: 150px;">{{item.insertTime}}</td>
                            <td style="width: 80px;">{{item.dipiOrderYesnoValue}}</td>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 dipiOrderContent 字段去除无用字符 如果字段长度大于60  就截取到60
            dipiOrderContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            //列表查询
            // ,pageList(dipiOrderTypes) {
            //     this.dipiOrderTypes = dipiOrderTypes;
            //     // 获取列表数据
            //     layui.http.request('dipiOrder/list', 'get', {
            //         page: 1,
            //         limit: this.limit,
            //         dipiOrderTypes: vue.dipiOrderTypes
            //     }, function(res) {
            ...
            //                         page: obj.curr,
            //                         limit: obj.limit,
            //                         dipiOrderTypes: this.dipiOrderTypes
            //                     }, function(res) {
            ...
            ,pay(id) {
                // 获取商品详情信息
                layui.http.requestJson(`dipiOrder/pay`, 'PPST',{
                    id:id
                }, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
            ...
            ,refund(id) {
                // 获取商品详情信息
                layui.http.request(`dipiOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        // 获取列表数据
        http.request('dipiOrder/list', 'get',{
            page: 1,
            limit: limit,
            // dipiOrderTypes: vue.dipiOrderTypes
        }, function(res) {
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('dipiOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            dipiOrderType: vue.dipiOrderType
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
       ...
       function zuowuTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=zuowu_types", 'get', {}, function (res) {
               if(res.code == 0){
       ...
       function dipiOrderYesnoTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=dipi_order_yesno_types", 'get', {}, function (res) {
               if(res.code == 0){
